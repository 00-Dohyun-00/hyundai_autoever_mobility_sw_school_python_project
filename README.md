# 네이버 국어사전 크롤링

Flask + Jinja2 기반 웹 화면에서 사용자가 검색어를 입력하면 Selenium을 이용해 네이버 국어사전을 검색하고, 검색 결과의 단어와 뜻을 가져와 화면에 표시하는 기능입니다.

단순히 검색 결과를 출력하는 것뿐만 아니라 검색 진행 상태, 검색 결과 단어 확인, 오류 안내, 최근 검색어, 메모 기능 등을 추가하여 검색 결과를 확인하고 정리할 수 있도록 구성했습니다.

---

## 주요 기능

- Selenium을 이용한 네이버 국어사전 검색
- 검색 결과의 뜻 자동 추출
- 실제 검색 결과 단어 확인
- 입력 검색어와 실제 결과 단어 비교
- 뜻별 개별 결과 카드 표시
- 네이버 국어사전 원문 링크 제공
- 검색 중 로딩 화면
- 크롤링 오류 유형별 안내
- 유지보수를 위한 상세 오류 로그
- 최근 검색어 최대 5개
- 최근 검색어 클릭 시 재검색
- 검색 결과 메모 기능
- 모든 검색 결과 한 번에 메모
- 메모 개별 삭제
- 메모 전체 초기화
- 메모 TXT 파일 저장

---

# 사용 기술

## Backend

- Python
- Flask
- Jinja2

## Crawling

- Selenium
- Chrome
- ChromeDriver

## Frontend

- HTML
- CSS
- JavaScript
- sessionStorage

---

# 파일 구성

크롤링 기능과 관련된 주요 파일은 다음과 같습니다.

```text
crawler/
├─ __init__.py
└─ crawler.py

templates/
└─ crawl_result.html
```

---

# 파일 역할

| 파일 | 역할 |
| --- | --- |
| `crawler/__init__.py` | crawler 폴더를 Python 패키지로 사용하기 위한 파일 |
| `crawler/crawler.py` | Selenium 실행, 네이버 국어사전 검색, 뜻 추출, 실제 결과 단어 확인, 오류 처리 |
| `templates/crawl_result.html` | 검색창, 검색 결과, 로딩 화면, 오류 화면, 최근 검색어, 메모장 UI |

---

# 크롤링 화면

크롤링 기능은 `/crawl-result` 화면에서 사용합니다.

사용자가 검색어를 입력하고 검색 버튼을 누르면 Selenium이 네이버 국어사전에 접속하여 검색 결과를 가져옵니다.

```text
/crawl-result
```

검색어가 포함된 경우 다음과 같은 형태로 요청됩니다.

```text
/crawl-result?keyword=사출
```

---

# 기본 검색 흐름

전체 크롤링 과정은 다음과 같습니다.

```text
사용자 검색어 입력
        ↓
검색 버튼 클릭
        ↓
Chrome Driver 실행
        ↓
네이버 국어사전 접속
        ↓
검색 입력창 확인
        ↓
검색어 입력
        ↓
ENTER
        ↓
검색 결과 로딩 대기
        ↓
뜻 추출
        ↓
실제 검색 결과 단어 확인
        ↓
Flask로 결과 반환
        ↓
Jinja2 화면 출력
```

---

# Selenium Driver

Chrome을 자동으로 실행하기 위해 Selenium의 Chrome WebDriver를 사용합니다.

```python
driver = webdriver.Chrome(
    options=options
)
```

현재 기본 설정에서는 Chrome 창이 화면에 표시되지 않는 Headless 모드를 사용합니다.

```python
options.add_argument(
    "--headless=new"
)
```

크롤링 동작을 직접 확인하고 싶은 경우 해당 코드를 주석 처리할 수 있습니다.

```python
# options.add_argument(
#     "--headless=new"
# )
```

Headless 모드를 해제하면 Selenium이 실제 Chrome 창을 실행하여 검색하는 과정을 직접 확인할 수 있습니다.

---

# 네이버 국어사전 접속

크롤링 대상 주소는 다음과 같습니다.

```python
NAVER_DICT_URL = "https://ko.dict.naver.com/#/main"
```

Selenium이 해당 페이지에 접속한 뒤 검색 입력창이 나타날 때까지 기다립니다.

검색창은 다음 XPath를 기준으로 확인합니다.

```python
SEARCH_XPATH = '//*[@id="ac_input"]'
```

---

# 검색어 입력

검색창을 찾은 뒤 사용자가 입력한 검색어를 전달합니다.

```python
search_box.clear()

search_box.send_keys(
    keyword
)

search_box.send_keys(
    Keys.ENTER
)
```

검색 버튼을 직접 클릭하는 대신 검색어 입력 후 `ENTER`를 전달하여 검색을 실행합니다.

---

# 검색 결과 대기

네이버 국어사전 검색 결과는 검색 직후 바로 만들어지지 않을 수 있기 때문에 Selenium의 `WebDriverWait`을 이용합니다.

검색 결과의 뜻 영역 또는 추가 검색 결과 버튼이 나타날 때까지 기다린 뒤 다음 작업을 진행합니다.

이를 통해 페이지가 완전히 표시되기 전에 HTML을 읽는 상황을 줄였습니다.

---

# 뜻 추출

검색 결과의 뜻은 다음 CSS Selector를 이용해 찾습니다.

```python
MEAN_SELECTOR = 'p.mean[lang="ko"]'
```

검색 페이지에서 해당 요소를 모두 가져온 뒤 각각의 텍스트를 읽습니다.

```python
mean_elements = driver.find_elements(
    By.CSS_SELECTOR,
    MEAN_SELECTOR
)
```

가져온 텍스트 중 다음 값은 제외합니다.

- 비어 있는 내용
- 읽을 수 없는 요소
- 이미 저장된 동일한 뜻

최종적으로 중복되지 않은 뜻 목록을 생성합니다.

---

# 검색 결과 표시

검색 결과는 뜻 하나마다 별도의 카드 형태로 표시합니다.

예:

```text
사출 뜻 1

첫 번째 뜻 내용

네이버 국어사전
[메모에 추가]
```

```text
사출 뜻 2

두 번째 뜻 내용

네이버 국어사전
[메모에 추가]
```

하나의 큰 영역에 모든 뜻을 표시하지 않고, 뜻 하나를 하나의 결과 단위로 나누어 확인할 수 있도록 구성했습니다.

---

# 네이버 국어사전 원문 확인

검색 결과 화면에는 네이버 국어사전 원문 링크도 제공합니다.

사용자가 결과를 직접 확인하고 싶은 경우:

```text
원문에서 확인 ↗
```

링크를 눌러 실제 네이버 국어사전 검색 결과로 이동할 수 있습니다.

---

# 실제 검색 결과 단어 확인

사용자가 입력한 검색어와 네이버 국어사전에서 실제로 표시되는 결과 단어가 항상 같다고 볼 수 없기 때문에 검색 결과의 실제 단어도 별도로 확인합니다.

뜻 영역을 기준으로 가까운 검색 결과 영역을 찾고, 해당 영역의 단어 텍스트를 가져옵니다.

검색 결과 단어를 정상적으로 가져오면 화면의 결과 제목으로 사용합니다.

예:

```text
사출 뜻 1
사출 뜻 2
사출 뜻 3
```

---

# 검색어와 실제 결과 단어 비교

사용자가 입력한 검색어와 실제 네이버 국어사전 결과 단어를 비교합니다.

비교할 때는 문자열 형식을 정리한 뒤 비교합니다.

```python
normalize_for_compare(
    keyword
)
```

검색어와 결과가 다른 경우에도 검색 결과 자체를 삭제하지 않습니다.

대신 화면 상단에 경고를 표시합니다.

예:

```text
검색어와 실제 검색 결과 단어가 다릅니다.

입력 검색어: 사추
네이버 국어사전 결과: 사출

아래 결과는 "사출"의 뜻입니다.
```

이를 통해 사용자는 자신이 입력한 검색어와 실제로 표시된 단어를 함께 확인할 수 있습니다.

---

# 실제 결과 단어를 찾지 못한 경우

실제 검색 결과 단어를 확인하지 못했다고 해서 정상적으로 가져온 뜻까지 삭제하지 않습니다.

결과 단어를 읽지 못한 경우 사용자가 처음 입력한 검색어를 대신 사용합니다.

```text
실제 결과 단어 확인 실패
        ↓
뜻 검색 결과는 유지
        ↓
입력 검색어를 결과 제목으로 사용
```

이 경우 전체 검색 실패가 아니라 부분 경고로 처리할 수 있도록 구성했습니다.

---

# 단어 더보기

검색 결과에 추가 뜻이 존재하는 경우 네이버 국어사전의 `단어 더보기` 버튼을 확인합니다.

```python
MORE_BUTTON_XPATH = '//*[@id="searchPage_word_more"]'
```

버튼이 존재하는 경우 클릭하여 추가 결과를 불러옵니다.

일반 클릭이 되지 않는 경우 JavaScript 클릭을 한 번 더 시도합니다.

```python
driver.execute_script(
    "arguments[0].click();",
    more_button
)
```

추가 결과가 정상적으로 나타나면 다시 뜻 전체를 읽어 최종 결과에 포함합니다.

---

# 검색 중 로딩 화면

Selenium은 실제 웹페이지에 접속하고 검색 결과가 표시될 때까지 기다려야 하기 때문에 일반적인 화면 이동보다 검색 시간이 길어질 수 있습니다.

검색 버튼을 누른 직후 다음 로딩 화면을 표시합니다.

```text
네이버 국어사전을 검색하고 있습니다

검색 결과와 뜻을 가져오는 중입니다.
잠시 기다려주세요.
```

검색 중에는:

- 로딩 Spinner 표시
- 검색 버튼 비활성화
- 검색 버튼을 `검색 중...`으로 변경

하도록 구성했습니다.

검색이 끝나고 페이지가 표시되면 다시 원래 상태로 돌아옵니다.

---

# 최근 검색어

검색 작업 중 이전 검색어를 쉽게 다시 사용할 수 있도록 최근 검색어 기능을 추가했습니다.

최대 5개까지 표시됩니다.

기능은 다음과 같습니다.

- 최근 검색어 최대 5개
- 같은 검색어 중복 저장 방지
- 다시 검색한 단어는 가장 앞으로 이동
- 최근 검색어 클릭 시 바로 재검색
- 개별 검색어 삭제
- 최근 검색어 전체 삭제

예:

```text
최근 검색어

[사출 ×] [금형 ×] [성형 ×] [수지 ×] [압력 ×]
```

---

# 최근 검색어 저장 범위

최근 검색어는 `sessionStorage`를 사용합니다.

현재 브라우저 세션 동안에는 검색 페이지가 다시 로딩되어도 최근 검색어가 유지됩니다.

다만 새로운 브라우저 세션이 시작되면 이전 검색 기록은 초기화됩니다.

장기간 검색 기록을 저장하기 위한 기능이 아니라 현재 검색 작업의 편의를 위한 기능입니다.

---

# 단어 메모장

검색 결과 중 필요한 뜻을 별도로 기록할 수 있도록 메모장 기능을 추가했습니다.

각 뜻 결과에 다음 버튼이 표시됩니다.

```text
메모에 추가
```

버튼을 누르면 해당 단어와 뜻이 화면 하단의 메모장에 추가됩니다.

---

# 메모 내용

메모에는 다음 정보를 저장합니다.

- 단어
- 뜻
- 출처
- 메모한 시간

예:

```text
사출

첫 번째 뜻 내용

출처 · 네이버 국어사전
```

---

# 중복 메모 방지

같은 단어와 같은 뜻이 이미 메모되어 있다면 다시 추가하지 않습니다.

```text
단어 동일
+
뜻 동일
        ↓
중복 메모로 판단
        ↓
추가하지 않음
```

이미 존재하는 메모를 다시 추가하려고 하면:

```text
이미 저장된 메모입니다.
```

라는 안내 메시지를 표시합니다.

---

# 모든 뜻 메모 추가

현재 검색 결과 전체를 한 번에 메모할 수도 있습니다.

```text
모든 뜻 메모에 추가
```

버튼을 누르면 현재 화면의 뜻을 확인하여 아직 메모되지 않은 결과만 추가합니다.

이미 저장된 결과는 중복해서 저장하지 않습니다.

---

# 메모 삭제

필요하지 않은 메모는 개별적으로 삭제할 수 있습니다.

각 메모 오른쪽의:

```text
×
```

버튼을 누르면 해당 메모가 삭제됩니다.

---

# 메모 초기화

메모 전체를 삭제하고 싶은 경우:

```text
메모 초기화
```

버튼을 사용할 수 있습니다.

실수로 전체 메모가 삭제되는 것을 줄이기 위해 삭제 전 확인창을 표시합니다.

```text
메모를 모두 삭제하시겠습니까?
```

확인을 선택하면 현재 메모 전체가 초기화됩니다.

---

# 메모 저장 범위

화면의 메모 역시 `sessionStorage`를 사용합니다.

따라서 현재 검색 작업 중에는 페이지가 다시 로딩되어도 메모를 계속 사용할 수 있습니다.

새로운 브라우저 세션에서는 초기화됩니다.

중요한 메모는 브라우저 세션을 종료하기 전에 TXT 파일로 저장할 수 있습니다.

---

# 메모 TXT 파일 저장

메모장에 기록한 내용을 TXT 파일 형태로 저장할 수 있습니다.

화면에서:

```text
파일로 저장
```

버튼을 누르면 현재 메모 내용을 텍스트 파일로 저장합니다.

파일명은 날짜를 포함하여 생성됩니다.

예:

```text
dictionary_memo_2026-08-27.txt
```

파일 내용은 다음과 같은 형태입니다.

```text
네이버 국어사전 검색 메모
========================================

1. 사출
뜻: 첫 번째 뜻 내용
출처: 네이버 국어사전
저장일시: 2026. 8. 27. 오전 11:20:00
----------------------------------------

2. 금형
뜻: 두 번째 뜻 내용
출처: 네이버 국어사전
저장일시: 2026. 8. 27. 오전 11:22:00
----------------------------------------
```

별도의 Flask 저장 Route를 추가하지 않고 현재 화면의 메모 데이터를 이용해 TXT 파일로 저장하도록 구성했습니다.

따라서 메모 파일 저장 기능을 위해 `app.py`에 추가적인 Route를 만들 필요가 없습니다.

---

# 오류 처리

웹 크롤링은 Chrome 실행 상태, 네트워크, 대상 웹사이트의 상태, 페이지 구조 등 여러 환경에 영향을 받을 수 있습니다.

따라서 검색 실패를 모두 같은 결과로 처리하지 않고 대략적인 오류 유형을 구분하여 화면에 표시합니다.

---

# 오류 유형

| 오류 코드 | 설명 |
| --- | --- |
| `EMPTY_KEYWORD` | 검색어가 입력되지 않은 경우 |
| `DRIVER_START_FAILED` | Chrome 실행에 실패한 경우 |
| `SITE_CONNECTION_FAILED` | 네이버 국어사전 접속에 실패한 경우 |
| `SEARCH_BOX_NOT_FOUND` | 검색 입력창을 찾지 못한 경우 |
| `SEARCH_INPUT_FAILED` | 검색어 입력 또는 검색 실행에 실패한 경우 |
| `ACCESS_BLOCKED` | 자동화 접근 제한이 의심되는 경우 |
| `NO_RESULTS` | 검색 결과가 없는 경우 |
| `RESULT_LOAD_TIMEOUT` | 검색 결과가 제한 시간 안에 나타나지 않은 경우 |
| `DOM_READ_FAILED` | 검색 결과 HTML 요소를 읽지 못한 경우 |
| `MEANING_NOT_FOUND` | 검색 페이지에서 뜻을 찾지 못한 경우 |
| `BROWSER_COMMUNICATION_FAILED` | Chrome과 Selenium의 연결에 문제가 발생한 경우 |
| `UNEXPECTED_ERROR` | 현재 분류에 포함되지 않은 예외가 발생한 경우 |

---

# 오류 화면

오류가 발생하면 사용자 화면에 다음 정보를 표시합니다.

```text
오류 코드
오류 제목
오류 설명
확인해볼 내용
```

예:

```text
RESULT_LOAD_TIMEOUT

검색 결과 로딩 지연

검색은 실행됐지만 제한 시간 안에
검색 결과가 나타나지 않았습니다.

확인해볼 내용

네트워크 또는 페이지 로딩이
지연되었을 수 있습니다.
잠시 후 다시 시도해주세요.
```

---

# 유지보수 로그

사용자 화면에는 오류의 종류와 확인 방법을 간단하게 표시합니다.

실제 예외의 상세 내용은 서버 실행 터미널에 별도로 기록합니다.

예:

```text
[Crawler 유지보수 로그] TimeoutException: ...
```

또는:

```text
[Crawler 유지보수 로그] WebDriverException: ...
```

이 로그는 동일한 오류가 반복되는 경우 실제 원인을 확인하고 크롤링 코드의 Selector나 오류 처리를 수정하기 위한 유지보수 자료로 사용할 수 있습니다.

---

# 부분 경고

일부 기능에서 문제가 발생해도 정상적으로 가져온 검색 결과가 존재하면 전체 검색을 실패로 처리하지 않습니다.

예를 들어:

- 실제 결과 단어를 확인하지 못한 경우
- 단어 더보기 버튼 클릭 실패
- 추가 결과 로딩 시간 초과

등이 있습니다.

이 경우 검색 결과는 유지하고 화면 아래쪽에 경고를 표시합니다.

```text
검색 결과 확보
       +
부가 기능 일부 실패
       ↓
검색 결과 유지
       +
경고 표시
```

---

# 전체 크롤링 흐름

```mermaid
flowchart TD

    A["검색어 입력"]
    --> B["검색 클릭"]

    B --> C["로딩 화면 표시"]

    C --> D["Chrome Driver 실행"]

    D --> E["네이버 국어사전 접속"]

    E --> F["검색창 탐색"]

    F --> G["검색어 입력"]

    G --> H["ENTER"]

    H --> I["검색 결과 로딩 대기"]

    I --> J["뜻 요소 탐색"]

    J --> K["뜻 텍스트 추출"]

    K --> L["실제 결과 단어 확인"]

    L --> M{"검색어와 결과 단어 일치?"}

    M -- "다름" --> N["불일치 경고 표시"]

    M -- "같음" --> O["정상 결과 표시"]

    N --> P["뜻별 결과 카드"]

    O --> P

    P --> Q["메모 추가"]

    P --> R["최근 검색어 관리"]

    Q --> S["TXT 파일 저장"]
```

---

# 크롤링 결과 데이터 구조

정상적으로 검색한 경우 `crawler.py`에서는 화면에서 사용할 정보를 하나의 Dictionary 형태로 반환합니다.

예:

```python
{
    "status": "ok",

    "input_keyword": "사출",

    "title": "사출",

    "url": "네이버 국어사전 검색 결과 URL",

    "meanings": [
        "첫 번째 뜻",
        "두 번째 뜻",
        "세 번째 뜻"
    ],

    "count": 3,

    "word_detected": True,

    "mismatch": False,

    "warnings": []
}
```

Jinja2에서는 이 값을 이용하여 검색 결과 카드를 생성합니다.

---

# 오류 결과 데이터 구조

오류가 발생한 경우에는 다음과 같은 형태로 화면에 전달합니다.

```python
{
    "status": "error",

    "error": {

        "code": "RESULT_LOAD_TIMEOUT",

        "title": "검색 결과 로딩 지연",

        "message": "검색 결과를 제한 시간 안에 확인하지 못했습니다.",

        "hint": "잠시 후 다시 시도해주세요."
    }
}
```

이를 통해 HTML에서는 오류 종류에 관계없이 동일한 오류 팝업 구조를 사용할 수 있습니다.

---

# 실행 방법

## 1. 가상환경 생성

Windows 기준:

```bash
python -m venv .venv
```

---

## 2. 가상환경 활성화

```bash
.venv\Scripts\activate
```

---

## 3. 필요한 패키지 설치

```bash
pip install -r requirements.txt
```

Selenium이 설치되어 있지 않은 경우:

```bash
pip install selenium
```

---

## 4. Flask 실행

```bash
python app.py
```

---

## 5. 브라우저 접속

```text
http://localhost:5000/crawl-result
```

---

# 사용 방법

1. 크롤링 화면에 접속합니다.
2. 검색창에 단어를 입력합니다.
3. `검색` 버튼을 클릭합니다.
4. 검색 중에는 로딩 화면이 표시됩니다.
5. 네이버 국어사전 검색 결과를 기다립니다.
6. 검색 완료 후 뜻별 결과 카드를 확인합니다.
7. 필요한 뜻은 `메모에 추가` 버튼을 사용합니다.
8. 이전 검색어는 최근 검색어 영역에서 다시 검색할 수 있습니다.
9. 필요한 메모를 모두 정리한 뒤 `파일로 저장` 버튼을 이용해 TXT 파일로 저장할 수 있습니다.

---

# 주의사항

## Chrome

Selenium을 이용하기 때문에 Chrome이 정상적으로 설치되어 있어야 합니다.

Chrome 실행에 문제가 있는 경우:

```text
DRIVER_START_FAILED
```

오류가 표시될 수 있습니다.

---

## 네트워크

네이버 국어사전은 외부 웹사이트이므로 인터넷 연결이 필요합니다.

네트워크 상태가 불안정한 경우:

```text
SITE_CONNECTION_FAILED
```

또는:

```text
RESULT_LOAD_TIMEOUT
```

오류가 발생할 수 있습니다.

---

## 네이버 국어사전 페이지 구조

현재 크롤링은 네이버 국어사전의 HTML 구조를 기준으로 구현되어 있습니다.

네이버에서 검색창이나 검색 결과 HTML 구조를 변경하면 기존 Selector가 더 이상 동작하지 않을 수 있습니다.

주요 Selector는 다음과 같습니다.

```python
SEARCH_XPATH = '//*[@id="ac_input"]'

MORE_BUTTON_XPATH = '//*[@id="searchPage_word_more"]'

MEAN_SELECTOR = 'p.mean[lang="ko"]'
```

검색 결과를 찾지 못하는 문제가 발생하면 우선 해당 Selector를 확인해야 합니다.

---

## 자동화 접근 제한

짧은 시간에 검색을 지나치게 많이 실행하면 웹사이트에서 자동화된 접근으로 판단할 가능성이 있습니다.

접근 제한 화면이 감지되는 경우:

```text
ACCESS_BLOCKED
```

오류로 분류합니다.

필요한 경우 Headless 모드를 해제하여 실제 Chrome 화면을 확인할 수 있습니다.

---

## 최근 검색어와 메모

최근 검색어와 메모는 현재 브라우저 세션 동안만 유지됩니다.

장기간 데이터를 저장하기 위한 기능은 아닙니다.

필요한 메모는 TXT 파일로 저장하여 별도로 보관할 수 있습니다.

---

# 요약

크롤링 기능은 다음 흐름을 하나의 화면에서 사용할 수 있도록 구성했습니다.

```text
검색어 입력
    ↓
네이버 국어사전 검색
    ↓
검색 진행 상태 확인
    ↓
실제 검색 결과 단어 확인
    ↓
뜻 추출
    ↓
검색어와 결과 비교
    ↓
뜻별 결과 표시
    ↓
최근 검색어
    ↓
메모
    ↓
TXT 파일 저장
```

Selenium을 이용한 기본적인 웹 크롤링 기능에 검색 상태 표시, 결과 검증, 오류 안내, 최근 검색어, 메모 기능을 추가하여 검색 결과를 확인하고 필요한 내용을 정리할 수 있도록 구성했습니다.
